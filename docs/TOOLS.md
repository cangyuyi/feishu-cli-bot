...
以下 16 个能力由大模型通过 function calling 自主调用（也可在规则模式/降级模式下被关键词触发）。每个工具最终都通过 `lark-cli` 与飞书交互。
...
- 工具名与 `schemas.TOOL_SCHEMA` 中的声明**一一对应**，由 `tests/test_tools.py` 的 `test_impl_keys_match_schema_names` 强制保证一致（16 对 16）。
